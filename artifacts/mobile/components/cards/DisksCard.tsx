    ...
    >
      <View style={styles.diskList}>
        {visibleDisks.map((disk, i) => {
          ...
          return (
            <View key={i} style={[styles.diskItem, i > 0 && styles.diskDivider]}>
              {/* Header */}
              <View style={styles.diskHeader}>
                ...
                {disk.temperature != null ? (
                  <TempBadge value={disk.temperature} />
                ) : null}
              </View>
              ...
              <View style={styles.usageRow}>
                ...
                <Text style={[styles.usagePct, { color: diskColor }]}>
              ...
              <View style={styles.spaceRow}>
                <Text style={styles.spaceUsed}>
                  {fmtMB(disk.used)} used
                </Text>
                <Text style={styles.spaceSep}>·</Text>
                <Text style={styles.spaceFree}>
                  {fmtMB(disk.free)} free
                </Text>
                <Text style={styles.spaceSep}>·</Text>
                <Text style={styles.spaceTotal}>
                  {fmtMB(disk.total)} total
                </Text>
              </View>
              ...
              {(disk.readSpeed > 0 || disk.writeSpeed > 0) && (
                <View style={styles.ioRow}>
                  <View style={styles.ioItem}>
                    ...
                    <Text style={[styles.ioVal, { color: "#2DD4BF" }]}>
                      {fmtSpeed(disk.readSpeed)}
                    </Text>
                  </View>
                  <View style={styles.ioSep} />
                  <View style={styles.ioItem}>
                    <Text style={styles.ioLabel}>Write</Text>
                    <Text style={[styles.ioVal, { color: "#F472B6" }]}>
                      {fmtSpeed(disk.writeSpeed)}
                    </Text>
                  </View>
                </View>
              )}
