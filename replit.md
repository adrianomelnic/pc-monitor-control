...
- `fetchMetrics` `catch` logs the actual reason (`Network error` / `Timeout` / `Parse error` / `HTTP 4xx`) under `__DEV__` so the next "stuck on Offline" report is diagnosable without a debugger.
